...
const addToCart = async (userId, productId, quantity) => {
    ...
    // Check stock availability (using inventory-first: stockTotal - stockReserved)
    const stockAvailable = product.stockTotal - product.stockReserved;
    if (stockAvailable < quantity) {
        const error = new Error(
            `Insufficient stock. Only ${stockAvailable} ${product.unit} available.`
        );
        error.statusCode = 400;
        throw error;
    }
...
const updateCartItem = async (userId, productId, quantity) => {
    ...
    // Check stock (using inventory-first: stockTotal - stockReserved)
    const stockAvailable = product.stockTotal - product.stockReserved;
    if (stockAvailable < quantity) {
        const error = new Error(
            `Insufficient stock. Only ${stockAvailable} ${product.unit} available.`
 ...
 * Checks all items for stock availability and MOQ
 * @param {string} userId - User's MongoDB ObjectId
...
const validateCartForCheckout = async (userId) => {
    ...
    for (const item of cart.items) {
        ...
        if (stockAvailable < item.quantity) {
            issues.push({
                ...
                message: `Only ${stockAvailable} ${product.unit} of ${product.name} available`,
            });
            continue;
        }
    ...
    return {
        isValid: issues.length === 0 && validItems.length > 0,
        issues,
...
const getCartSummary = async (userId) => {
    ...
    for (const item of cart.items) {
        if (item.product) {
            const stockAvailable = item.product.stockTotal - item.product.stockReserved;
    ...
    return {
        ...
        items: cart.items.map((item) => ({
            ...
            stockAvailable: item.product ? item.product.stockTotal - item.product.stockReserved : 0,
