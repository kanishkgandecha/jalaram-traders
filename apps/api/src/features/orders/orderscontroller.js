...
/**
 * @desc    Mark payment as submitted
 * @route   POST /api/v1/orders/:id/payment-submitted
 * @access  Private (Order Owner)
 */
const markPaymentSubmitted = async (req, res) => {
    try {
        const order = await ordersService.markPaymentSubmitted(
            req.params.id,
            req.user._id,
            req.body
        );

        sendSuccess(res, 200, 'Payment marked as submitted', { order });
    } catch (error) {
        console.error('Mark payment submitted error:', error);
        sendError(res, error.statusCode || 500, error.message);
    }
};
...
const confirmPayment = async (req, res) => {
    try {
        const order = await ordersService.confirmPayment(req.params.id, req.user._id);
...
const assignEmployee = async (req, res) => {
    try {
        ...
        const order = await ordersService.assignEmployee(
            req.params.id,
            employeeId,
            req.user._id
        );

        sendSuccess(res, 200, 'Employee assigned successfully', { order });
    } catch (error) {
...
const getEmployeeOrders = async (req, res) => {
    try {
        const { orders, pagination } = await ordersService.getEmployeeOrders(
            req.user._id,
            req.query
        );
